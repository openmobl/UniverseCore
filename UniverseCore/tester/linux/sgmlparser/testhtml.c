...
//	sgmlParserExtensionSetParam(&sgml, SGML_EXTENSION_HTML_PARAM_SETKNOWNTAGS, (void *)knownTags);
    ...
    domNodeSerializeToString(sgmlExtensionHtmlGetDocument(&sgml), &out);
    ...
    domNodeSerializeTreeToString(sgmlExtensionHtmlGetDocument(&sgml), &out, 0);
    printf("\nTree:\n");
    printf(out);
    printf("\n");
    ...
    domNodeSerializeTreeToString(sgmlExtensionHtmlGetDocument(&sgml), &out, 1);
    printf("\nTree (data shown):\n");
    printf(out);
    printf("\n");

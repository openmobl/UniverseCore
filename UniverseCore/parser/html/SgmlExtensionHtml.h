...
typedef struct _sgml_extension_html {
    ...
    void (*saveDivert)(struct _sgml_parser *parser, unsigned long newIndex, unsigned long oldState, unsigned long newState, const DOMString lastBuffer, unsigned long lastBufferSize);    
...
void sgmlExtensionHtmlTextNew(SGML_PARSER *parser, void *userContext, const DOMString text);
void sgmlExtensionHtmlCommentNew(SGML_PARSER *parser, void *userContext, const DOMString comment);
...
typedef struct _sgml_extension_html_cleanup {
    int     mode;
...
/**
 * Cleanup the markup based on the passed mode.
 *
 * @param node The first node in the document. Should be DOM_NODE_DOCUMENT
 * @param mode The mode to use (Quirks or Strict).
 */

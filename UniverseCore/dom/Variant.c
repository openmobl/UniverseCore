...
void variantSet(enum VariantType type, void *value, unsigned long valueSize, void *userParam, void (*setter)(void *userParam, const DOMString string))
{
    switch (type)
    {
        ...
        case VARIANT_TYPE_SIGNED_INTEGER:
            if (valueSize >= sizeof(signed long))
            {
                ...
                stringNPrintF(trans, sizeof(trans) - 1, "%li", *(signed long *)value);
        ...
        case VARIANT_TYPE_UNSIGNED_INTEGER:
            if (valueSize >= sizeof(unsigned long))
            {
                ...
                stringNPrintF(trans, sizeof(trans) - 1, "%lu", *(unsigned long *)value);
        ...
        case VARIANT_TYPE_SIGNED_SHORT:
            if (valueSize >= sizeof(signed short))
            {
                ...
                stringNPrintF(trans, sizeof(trans) - 1, "%d", *(signed short *)value);
        ...
        case VARIANT_TYPE_UNSIGNED_SHORT:
            if (valueSize >= sizeof(signed short))
            {
                ...
                stringNPrintF(trans, sizeof(trans) - 1, "%u", *(unsigned short *)value);
        ...
        case VARIANT_TYPE_DOUBLE:
            if ((valueSize >= sizeof(float)) || (valueSize >= sizeof(double)))
            {
                ...
                stringNPrintF(trans, sizeof(trans) - 1, "%f", *(float *)value);
...
void *variantGet(enum VariantType type, const DOMString string)
{
    if (!string)
        return NULL;
    ...
    switch (type)
    {
        case VARIANT_TYPE_STRING:
            return (void *)string;
        case VARIANT_TYPE_SIGNED_INTEGER:
            return (void *)stringToLong(string, NULL, 10);
        case VARIANT_TYPE_UNSIGNED_INTEGER:
            return (void *)stringToULong(string, NULL, 10);
        case VARIANT_TYPE_SIGNED_SHORT:
            {
                signed short  a;
#ifdef PALMOS
                char          *temp = stringDup(string);
                
                if (!temp) {
                    a = (signed short)stringAToI(string);
                } else {
                    stringMakeInt(temp);
                    
                    a = (signed short)stringAToI(temp);
        ...
        case VARIANT_TYPE_UNSIGNED_SHORT:
            {
                ...
                if (!temp) {
                    a = (unsigned short)stringAToI(string);
                } else {
                    stringMakeInt(temp);
                    
                    a = (unsigned short)stringAToI(temp);
                    memMgrChunkFree(temp);
                }
#else
                sscanf(string, "%hu", &a);
        ...
        case VARIANT_TYPE_FLOAT:
            {
                VARIANT_VALUE *value = (VARIANT_VALUE *)memMgrChunkNew(sizeof(VARIANT_VALUE));

                if (value)
                {
                    memMgrChunkSet(value, 0, sizeof(VARIANT_VALUE));

                    value->flt = stringAToF(string);
        ...
        case VARIANT_TYPE_BINARY_B64:
            {
                ...
                if (value)
                {
                    ...
                    value->binary = (unsigned char *)_variantBase64Decode((const unsigned char *)string, stringLen(string), &value->length);
                }
                return value;
            }
            break;
...
char *_variantBase64Encode(const unsigned char *src, unsigned long srcLength)
{
    ...
    dest = (char *)memMgrChunkNew(destLength);

    if (!dest)
        return NULL;

    memMgrChunkSet(dest, 0, destLength);
...
{
    char code[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    unsigned long srcLen = stringLen( (const char *) src );
    ...
    if (!(dest = (char *)memMgrChunkNew(destLen)))
        return NULL;

    memMgrChunkSet( isValid, 0, sizeof(isValid) );
    memMgrChunkSet( decodeValue, 0, sizeof(isValid) );

...
/*
#define DOM_NODE_TYPE_INVALID   0x0000
...
#define INVALID_NODE                    0x0000
#define ELEMENT_NODE                    0x0001
#define ATTRIBUTE_NODE                  0x0002
#define TEXT_NODE                       0x0003
#define CDATA_SECTION_NODE              0x0004
#define ENTITY_REFERENCE_NODE           0x0005
#define ENTITY_NODE                     0x0006
#define PROCESSING_INSTRUCTION_NODE     0x0007
#define COMMENT_NODE                    0x0008
#define DOCUMENT_NODE                   0x0009
#define DOCUMENT_TYPE_NODE              0x0010
#define DOCUMENT_FRAGMENT_NODE          0x0011
#define NOTATION_NODE                   0x0012
#define XPATH_NAMESPACE_NODE            0x0013
...
typedef DOM_NODE DOMNodeType;
typedef DOMNodeType *DOMNodePtr;
typedef DOMNodeType DOMNode;
...
/**
 * Appends a sibling to a node after a specified node.
 *
 * @param  node    [in] The node to have a sibling appended to it.
 * @param  after   [in] The node to append after.
 * @param  sibling [in] The sibling to append.
 */
void domNodeAppendChildAfter(DOM_NODE *parent, DOM_NODE *after, DOM_NODE *child);
/**
 * Appends a sibling to a node as the first child.
 *
 * @param  node    [in] The node to have a sibling appended to it.
 * @param  sibling [in] The sibling to append.
 */
 ...
 *
 * @param  node [in] The node to get the parent of.
 * @return If the node has a parent, a valid node will be returned.  Otherwise, NULL is returned.
 */
 ...
 */
struct _dom_node_list *domNodeFindNodesByName(DOM_NODE *node, const DOMString name);
void domNodeFindNodesByName_r(struct _dom_node_list *nodeList, DOM_NODE *node, const DOMString name);
...
/**
 * Serializes a given node to a string.  This string must be deallocate with free() when it is dnoe being used. Displays the unformatted tree.
 *
 * @param  node     [in]  The node to serialize.
 * @param  string   [out] A pointer to a pointer that will hold the null terminated SGML string.
 * @return On success, a value greater than zero is returned that represents the length of the SGML string.  Otherwise, zero is returned.
...
void domNodeSerializeTreeToString_r(DOM_NODE *node, char **string, unsigned long *stringLength, char *indent, int showValues, int safe);

...
{
    DOM_NODE_LIST *nodeList = (DOM_NODE_LIST *)memMgrChunkNew(sizeof(DOM_NODE_LIST));

    memMgrChunkSet(nodeList, 0, sizeof(DOM_NODE_LIST));
...
void domNodeListDestroy(DOM_NODE_LIST *nodeList)
{
    ...
    while (nodeList->firstNode)
    {
        ...
        memMgrChunkFree(nodeList->firstNode);
...
void domNodeListAddNode(DOM_NODE_LIST *nodeList, DOM_NODE *node)
{
    ...
    new       = (DOM_NODE_LIST_NODE *)memMgrChunkNew(sizeof(DOM_NODE_LIST_NODE));
    new->node = node;
    new->next = NULL;
